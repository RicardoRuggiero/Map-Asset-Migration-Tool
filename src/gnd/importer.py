...
class GND_OT_ImportOperator(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    ...
    def import_gnd(filepath, options: GndImportOptions):
        ...
        for i, texture in enumerate(gnd.textures):
            ...
            # material.specular_intensity = 0.0  # Apagado/Comentado pois é obsoleto
            material.use_nodes = True
            materials.append(material)

            bsdf = material.node_tree.nodes['Principled BSDF']
            # Atualizado para a nomenclatura da API do Blender 4.0+
            bsdf.inputs['Specular IOR Level'].default_value = 0.0
            texImage = material.node_tree.nodes.new('ShaderNodeTexImage')
            material.node_tree.links.new(bsdf.inputs['Base Color'], texImage.outputs['Color'])

            ''' Load diffuse texture. '''
            # Força o caminho relativo à pasta onde o .rsw está
            texpath = os.path.join(directory_name, 'texture', texture_path.replace('\\', os.sep))
            
            try:
                texImage.image = bpy.data.images.load(texpath, check_existing=True)
            except RuntimeError as e:
                # Agora o terminal do VS Code vai gritar qual arquivo exato ele não achou
                print(f"Erro ao carregar textura: {texpath}")
                pass
